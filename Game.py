...
def Admin():
    ...
    return render_template("admin.html", player = json_player, score = json_score, max = max)
...
def bestscore():
    ...
    user_id = db.session.query(Score).filter(Score.score == max).all()
    return render_template("admin.html", player_best = json_player, score_best = json_score)
@app.route("/game")
@login_required
def Game():
    admin = ""
